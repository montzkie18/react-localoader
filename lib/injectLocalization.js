...
function isTranslatableAttribute(node) {
  ...
  return node.type === 'JSXAttribute' 
    ...
    && node.value && (isTextString(node.value) || isTextExpression(node.value));
}
...
module.exports = function injectLocalization(source) {
  ...
  var componentLib = config.componentImport.match(/['"](.+?)['"]/)[1];
  if(didInjectElement && !code.includes(componentLib))
    code = config.componentImport + "\n" + code;

  var i18nLib = config.i18nImport.match(/['"](.+?)['"]/)[1];
  if(didInjectAttribute && !code.includes(i18nLib))
